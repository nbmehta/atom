...
class RWMB_File_Field extends RWMB_Field
{
	...
	static function admin_enqueue_scripts()
	{
		...
		wp_localize_script( 'rwmb-file', 'rwmbFile', array(
			'maxFileUploadsSingle' => esc_html__( 'You may only upload maximum %d file', 'emallshop' ),
			'maxFileUploadsPlural' => esc_html__( 'You may only upload maximum %d files', 'emallshop' ),
		) );
	}
	...
	static function wp_ajax_delete_file()
	{
		...
		if ( $success )
			...
			wp_send_json_error( esc_html__( 'Error: Cannot delete file', 'emallshop' ) );
	}
	...
	{
		$i18n_title = apply_filters( 'rwmb_file_upload_string', _x( 'Upload Files', 'file upload', 'emallshop' ), $field );
		$i18n_more  = apply_filters( 'rwmb_file_add_string', _x( '+ Add new file', 'file upload', 'emallshop' ), $field );
	...
	{
		$i18n_delete = apply_filters( 'rwmb_file_delete_string', _x( 'Delete', 'file upload', 'emallshop' ) );
		$i18n_edit   = apply_filters( 'rwmb_file_edit_string', _x( 'Edit', 'file upload', 'emallshop' ) );
		$item        = '
		<li id="item_%s">
