...
if (!class_exists('EMALLSHOP_SIDEBAR')) {
	...
	class EMALLSHOP_SIDEBAR{
		...
		public function enqueue_assets() {
			...
			wp_enqueue_script('custom_sidebar_js' , EMALLSHOP_URI . '/framework/assets/js/custom_sidebar.js');
			wp_enqueue_style( 'custom_sidebar_css' , EMALLSHOP_URI . '/framework/assets/css/custom_sidebar.css');
		}
		...
		public function add_field() {
			...
			$output .= "\n  <h3>". $this->title ."</h3>";
			$output .= "\n    <p><input size='30' type='text' value='' placeholder = '". esc_html__('Enter Name for new Widget Area', 'emallshop') ."' name='form-add-widget' /></p>";
			$output .= "\n    <input class='button button-primary' type='submit' value='". esc_html__('Add Widget Area', 'emallshop') ."' />";
